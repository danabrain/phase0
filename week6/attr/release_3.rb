...
class Profile
  ...
  #yay, now @age has an attribute writer!  This means the setter method change_my_age=(new_age) has been written for us in one line of code.  It also means that when we want to change the value of @age from outside the Class, all we have to do is call .age on our object and set that equal to our desired value (no longer have to call change_my_age and pass in an argument).
